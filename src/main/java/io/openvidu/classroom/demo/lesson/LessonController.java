...
public class LessonController {
	...
	public ResponseEntity<Lesson> modifyLesson(@RequestBody Lesson lesson) {
		...
		Lesson c = lessonRepository.findById(lesson.getId()).get();
	...
	public ResponseEntity<AddAttendersResponse> addAttenders(@RequestBody String[] attenderEmails,
		...
		for (User attender : newPossibleAttenders) {
			boolean newAtt = true;
			if (!attender.getLessons().contains(c))
				attender.getLessons().add(c);
			else
				newAtt = false;
		...
		// relationship is missing in a Lesson or a User)
	...
	public ResponseEntity<Set<User>> deleteAttenders(@RequestBody Lesson lesson) {
		...
		userRepository.saveAll(lessonAttenders);
	...
	// Authorization checking for editing and deleting lessons (the teacher must own
	// the Lesson)
